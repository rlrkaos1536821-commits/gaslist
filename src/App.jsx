...
import { compressImage, PHOTO_SIZE_WARNING_BYTES } from './utils/imageUtils.js';
...
function flattenPhotosByItem(photosByItem = {}) {
  return Object.values(photosByItem).flat().sort((first, second) => {
    const firstIndex = evaluationItems.findIndex((item) => item.id === first.evaluationItemId);
    const secondIndex = evaluationItems.findIndex((item) => item.id === second.evaluationItemId);
...
function getPhotoErrorMessage(error) {
  if (error?.name === 'QuotaExceededError') {
    return '사진 저장 공간이 부족합니다. 일부 사진을 삭제한 뒤 다시 시도해주세요.';
  }
...
export default function App() {
  ...
  const loadPhotosForTask = async (taskId) => {
    if (!taskId) {
      setPhotosByItem({});
      return;
    }
    ...
    try {
      ...
      setPhotosByItem(groupPhotosByItem(photos));
    } catch (error) {
      console.error('Failed to load photos.', error);
      setPhotosByItem({});
      setNotice('저장된 사진을 불러오지 못했습니다.');
    }
  };
  ...
  const handleAddPhotos = async (item, files) => {
    if (!currentEvaluationId) {
      setNotice('작업을 먼저 시작한 뒤 사진을 첨부해주세요.');
      return;
    }

    const existingPhotos = photosByItem[item.id] ?? [];
    const availableSlots = MAX_PHOTOS_PER_ITEM - existingPhotos.length;

    if (availableSlots <= 0) {
      setNotice(`사진은 평가항목당 최대 ${MAX_PHOTOS_PER_ITEM}장까지 첨부할 수 있습니다.`);
      return;
    }

    const targetFiles = files.slice(0, availableSlots);
    if (files.length > availableSlots) {
      setNotice(`최대 ${MAX_PHOTOS_PER_ITEM}장까지만 저장되어 초과 사진은 제외했습니다.`);
    } else {
      setNotice('');
    }

    setPhotoProcessing((current) => ({
      ...current,
      [item.id]: `사진 압축 중 0/${targetFiles.length}`,
    }));
    ...
    try {
      ...
      for (const [index, file] of targetFiles.entries()) {
        setPhotoProcessing((current) => ({
          ...current,
          [item.id]: `사진 압축 중 ${index + 1}/${targetFiles.length}`,
        }));

        const compressed = await compressImage(file);
        const savedPhoto = await savePhoto({
          taskId: currentEvaluationId,
          evaluationItemId: item.id,
          category: item.category,
          order: existingPhotos.length + savedPhotos.length,
        ...
        setPhotosByItem((current) => ({
          ...current,
          [item.id]: [...(current[item.id] ?? []), savedPhoto],
        }));
      }

      const totalPhotoSize = flattenPhotosByItem({
        ...photosByItem,
        [item.id]: [...existingPhotos, ...savedPhotos],
      }).reduce((sum, photo) => sum + (photo.size ?? 0), 0);

      if (totalPhotoSize > PHOTO_SIZE_WARNING_BYTES) {
        setNotice('사진 용량이 커졌습니다. Excel 생성은 가능하지만 공유 앱에 따라 시간이 걸릴 수 있습니다.');
      }
    } catch (error) {
      console.error('Failed to add photos.', error);
      setNotice(getPhotoErrorMessage(error));
    } finally {
      setPhotoProcessing((current) => {
        const next = { ...current };
        delete next[item.id];
  ...
  const handleDeletePhoto = async (photo) => {
    ...
    try {
      ...
      const nextPhotos = await updatePhotoOrder(photo.taskId, photo.evaluationItemId);
      setPhotosByItem((current) => ({
        ...current,
        [photo.evaluationItemId]: nextPhotos,
      }));
  ...
  const createExcelForAction = async () => createEvaluationExcelBlob({
    ...
    photos: flattenPhotosByItem(photosByItem),
  });
  ...
  const shareStoredEvaluationExcel = async () => {
    ...
    try {
      const taskPhotos = await getPhotosByTask(selectedTask.id);
      const excel = await createEvaluationExcelBlobFromEvaluation(selectedTask, taskPhotos);
  ...
  const deleteSelectedTask = async () => {
    ...
    try {
      await deletePhotosByTask(selectedTask.id);
  ...
  const deleteTaskFromList = async (task) => {
    ...
    try {
      await deletePhotosByTask(task.id);
